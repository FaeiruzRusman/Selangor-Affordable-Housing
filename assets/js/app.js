...
async function load(){
  try{
    ...
    document.getElementById('coverageBadge').textContent=`${projects.length} verified/traceable project records`;
...
function buildFilters(){
  ...
  [['schemeFilter','scheme'],['districtFilter','district'],['statusFilter','status']].forEach(([id,k])=>{
...
function renderAll(){
 ...
 renderWaiting(); renderDistrict(); renderPipeline(); renderFiltered();
}
...
function renderFiltered(){const ps=filtered();document.getElementById('recordCount').textContent=`${ps.length} records`;document.getElementById('projectTable').innerHTML=ps.map(p=>`<tr><td><b>${p.project_name}</b><br><small>${p.project_id}</small></td><td>${safe(p.scheme)}</td><td>${safe(p.district)}</td><td>${safe(p.pbt)}</td><td>${fmt(p.units)}</td><td>${p.price_min_rm?money(p.price_min_rm):'—'}</td><td><span class="status-pill ${statusClass(p.status)}">${safe(p.status)}</span></td><td>${safe(p.expected_completion)}</td><td class="verification">${safe(p.verification)}</td></tr>`).join('')}
